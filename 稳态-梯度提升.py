# -*- coding: utf-8 -*-
"""
梯度提升回归 (GBDT) 预测“热感觉_数值”
- 训练/测试 = 8:2
- 输出：
  metrics.txt（R² / RMSE / MAE）
  feature_importance.(png/pdf, csv)
  confusion_matrix_counts.(png/pdf)
  confusion_matrix_norm.(png/pdf)
  classification_report.txt（基于回归结果四舍五入后的等级）
"""

from pathlib import Path
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
from matplotlib import font_manager
from matplotlib.ticker import FuncFormatter
from sklearn.model_selection import train_test_split
from sklearn.ensemble import GradientBoostingRegressor
from sklearn.metrics import (r2_score, mean_squared_error, mean_absolute_error,
                             confusion_matrix, classification_report)

# ========= 路径 =========
CSV_PATH = Path(r"D:\00 读研\04 组会\12 小米可穿戴项目\00 实验数据\超级大实验\任务记录\起止时间汇总\task_stable_windows_mapped-without25_with_tempstats_plus_signals_votes_with_demo.csv")
OUT_DIR  = Path(r"D:\00 读研\04 组会\12 小米可穿戴项目\00 实验数据\超级大实验\模型输出\GBDT_thermal_sensation2")
OUT_DIR.mkdir(parents=True, exist_ok=True)

# ========= 中文字体（可选）=========
for fam in ["Microsoft YaHei","SimHei","PingFang SC","Heiti SC","Noto Sans CJK SC","Source Han Sans SC"]:
    if fam in {f.name for f in font_manager.fontManager.ttflist}:
        plt.rcParams["font.sans-serif"] = [fam]; break
plt.rcParams["axes.unicode_minus"] = False

def read_csv_guess(p: Path) -> pd.DataFrame:
    try:
        return pd.read_csv(p, encoding="utf-8-sig")
    except UnicodeDecodeError:
        return pd.read_csv(p, encoding="gbk")

# ========= 数据读取与清洗 =========
feature_cols = [
    "temp_mean",
    "back_temp℃_mean",
    "hand_temp℃_mean",
    "leg_temp℃_mean",
    "neck_temp℃_mean",
    "mTSK_mean",
]
target_col = "热感觉_数值"

df = read_csv_guess(CSV_PATH)
df = df[feature_cols + [target_col]].copy()

# 转数值 & 去缺失
for c in feature_cols + [target_col]:
    df[c] = pd.to_numeric(df[c], errors="coerce")
df = df.dropna()

X = df[feature_cols].values
y = df[target_col].values

# ========= 划分数据（8:2）=========
X_train, X_test, y_train, y_test = train_test_split(
    X, y, test_size=0.2, random_state=180
)

# ========= 训练模型（GBDT）=========
# 可按需调参：n_estimators、learning_rate、max_depth、subsample 等
gbdt = GradientBoostingRegressor(
    n_estimators=1000,
    learning_rate=0.05,
    max_depth=9,
    random_state=21
)
gbdt.fit(X_train, y_train)

# ========= 回归评估 =========
y_pred = gbdt.predict(X_test)
r2   = r2_score(y_test, y_pred)
rmse = float(np.sqrt(mean_squared_error(y_test, y_pred)))
mae  = float(mean_absolute_error(y_test, y_pred))

with open(OUT_DIR / "metrics.txt", "w", encoding="utf-8") as f:
    f.write("梯度提升回归 (GBDT) - 预测 热感觉_数值\n")
    f.write(f"样本量：全量={len(df)}, 训练={len(y_train)}, 测试={len(y_test)}\n\n")
    f.write(f"R²   = {r2:.4f}\n")
    f.write(f"RMSE = {rmse:.4f}\n")
    f.write(f"MAE  = {mae:.4f}\n")
print("指标已写入：", OUT_DIR / "metrics.txt")

# ========= 特征重要性 =========
importances = gbdt.feature_importances_
fi = (pd.Series(importances, index=feature_cols).sort_values(ascending=True))
fi.to_csv(OUT_DIR / "feature_importance.csv", encoding="utf-8-sig")

plt.figure(figsize=(7,5), dpi=150)
plt.barh(fi.index, fi.values, color="#5DADE2", edgecolor="black")
plt.xlabel("特征重要性")
plt.title("GBDT 特征重要性")
plt.tight_layout()
plt.savefig(OUT_DIR / "feature_importance.png", dpi=300)
plt.savefig(OUT_DIR / "feature_importance.pdf")
plt.close()

# ========= 将回归输出四舍五入为等级，画混淆矩阵 =========
classes = np.array([-3, -2, -1, 0, 1, 2, 3], dtype=int)

def clip_round_to_classes(arr):
    arr = np.rint(arr).astype(int)
    return np.clip(arr, classes.min(), classes.max())

y_true_cls = clip_round_to_classes(y_test)
y_pred_cls = clip_round_to_classes(y_pred)

cm = confusion_matrix(y_true_cls, y_pred_cls, labels=classes)                         # 计数
cm_norm = confusion_matrix(y_true_cls, y_pred_cls, labels=classes, normalize="true")  # 行归一化(0-1)

# 保存分类报告（基于离散化后的标签）
report = classification_report(y_true_cls, y_pred_cls, labels=classes, digits=3, zero_division=0)
with open(OUT_DIR / "classification_report.txt", "w", encoding="utf-8") as f:
    f.write("（回归输出四舍五入至等级后的）混淆矩阵分类报告\n")
    f.write(report)

def plot_cm(mat, title, cmap="Blues", fmt="d", vmin=None, vmax=None, fname="cm.png"):
    fig, ax = plt.subplots(figsize=(6.6, 5.3), dpi=150)
    im = ax.imshow(mat, cmap=cmap, vmin=vmin, vmax=vmax)
    ax.set_title(title)
    ax.set_xlabel("预测等级")
    ax.set_ylabel("真实等级")
    ax.set_xticks(range(len(classes))); ax.set_yticks(range(len(classes)))
    ax.set_xticklabels(classes); ax.set_yticklabels(classes)

    # 数值标注
    for i in range(mat.shape[0]):
        for j in range(mat.shape[1]):
            val = mat[i, j]
            txt = f"{val:.0f}" if fmt == "d" else f"{val*100:.0f}%"
            ax.text(j, i, txt, ha="center", va="center",
                    color=("white" if (fmt=="d" and val>(mat.max()*0.6)) else "black"),
                    fontsize=10)
    cbar = plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    if fmt == "p":  
        cbar.ax.yaxis.set_major_formatter(
            FuncFormatter(lambda x, pos: f"{x*100:.0f}")  # ← 显示 0–100
        )
        cbar.set_label("行百分比(%)")
    plt.tight_layout()
    plt.savefig(OUT_DIR / (fname + ".png"), dpi=300)
    plt.savefig(OUT_DIR / (fname + ".pdf"))
    plt.close()

# 保存两张矩阵：计数 & 行百分比
plot_cm(cm,      "梯度提升 混淆矩阵",   cmap="Blues",
        fmt="d", vmin=0, vmax=cm.max(), fname="confusion_matrix_counts")
plot_cm(cm_norm, "梯度提升 混淆矩阵（行百分比）", cmap="Blues",
        fmt="p", vmin=0, vmax=1,        fname="confusion_matrix_norm")

print(f"[DONE] 输出目录：{OUT_DIR}")
print(f"R²={r2:.3f} | RMSE={rmse:.3f} | MAE={mae:.3f}")
